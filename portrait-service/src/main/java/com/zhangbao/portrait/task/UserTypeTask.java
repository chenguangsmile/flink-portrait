...
import com.zhangbao.portrait.map.UserTypeMap;
import com.zhangbao.portrait.reduce.UserTypeReduce;
import com.zhangbao.portrait.reduce.UserTypeSink;
...
public class UserTypeTask {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<UserTypeInfo> flatMap = input.flatMap(new UserTypeMap());

        DataStream<UserTypeInfo> reduce = flatMap.keyBy("groupField").timeWindowAll(Time.seconds(2)).reduce(new UserTypeReduce());

        reduce.addSink(new UserTypeSink());

        env.execute("user type analyse");
    }
}
